...
const RegisterPage: React.FC = () => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        if (!isEmail(email) && !isPhone(email)) {
            setError(
                'Введите корректную почту или номер телефона. Почта должна быть в формате user@example.com, телефон — 8XXXXXXXXXX',
            );
            return;
        }
        ...
        if (password.length < 6 || !isStrongPassword(password)) {
            setError(
                'Пароль должен быть не менее 6 символов и содержать хотя бы одну маленькую латинскую букву и одну цифру',
            );
            return;
        }
                ...
                <form onSubmit={handleSubmit} className="register-form">
                    ...
                    {error && (
                        <div
                            ...
                            style={{ color: ' #FF9500', marginBottom: 12 }}
                        >
                            {error}
                        </div>
                    )}
